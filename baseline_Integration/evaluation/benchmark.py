...
from config.params import POLY_MODULUS_DEGREE, SIMILARITY_THRESHOLD
...
def benchmark(config):
        ...
        config: dict，包含以下字段（示例）：
            {
                ...
                "k": 50,                      # 聚类数（若为0则线性搜索）
                "tau": 0.9,                   # 相似度阈值
                "fuzzy_ratio": 0.3,           # 正样本查询单字符 typo 比例
                "fuzzy_seed": 42,             # 模糊化随机种子
    ...
    if use_mock:
        # 模拟 B 的离线产物
        centroids = np.random.randn(config["k"], config["el_cluster"]).astype(
            np.float64
        )
        cluster_matrix = np.random.randn(config["k"], 300, config["el_match"]).astype(
            np.float64
        )
        ...
        k_mode = config.get("k_mode", k_config if k_config else "sqrt")
    ...
    # 2.5. 校验 he_batch_size 配置
    raw_he_batch = config.get("he_batch_size", config.get("batch_size", 0))
    if raw_he_batch is None:
        he_batch_size = 0
    elif isinstance(raw_he_batch, bool):
        raise ValueError(f"Invalid he_batch_size: {raw_he_batch}")
    elif isinstance(raw_he_batch, int):
        max_batch = POLY_MODULUS_DEGREE // 2
        if raw_he_batch < 0 or raw_he_batch > max_batch:
            raise ValueError(
                f"he_batch_size must be between 0 and {max_batch}, got {raw_he_batch}"
            )
        he_batch_size = raw_he_batch
    else:
        raise ValueError(f"Invalid he_batch_size: {raw_he_batch}")

    # 3. 在线阶段（对每个查询执行协议）
    num_queries = len(names_A)
    print(
        f"Running online phase for {num_queries} queries (he_batch_size={he_batch_size})..."
    )
    # 通信量统计
    total_sent_bytes = 0
    total_recv_bytes = 0
    ...
    if he_batch_size > 0 and not use_mock:
        # V2 query-by-candidate 2-D slot tiling 分块路径
        ...
        # 公开 context 在整个会话中仅计入一次
        total_sent_bytes += len(public_context_bytes)

        for start_idx in range(0, num_queries, he_batch_size):
            chunk_names = names_A[start_idx : start_idx + he_batch_size]

            r1_start = time.perf_counter()
            req1, party_a_state = prepare_tiled_query_batch(
                chunk_names,
                scaler_mean,
                scaler_scale,
            ...
            wire_req1 = serialize_tiled_first_round_request(req1)
            total_sent_bytes += sum(len(b) for b in wire_req1.encrypted_query_200)

            bytes_sim_scores = compare_tiled_batch_to_centroids(
                wire_req1,
                centroids,
                serialize_output=True,
            )
            total_recv_bytes += sum(len(b) for b in bytes_sim_scores)

            req2, _ = choose_clusters_and_build_tiled_request(
                bytes_sim_scores,
                party_a_state,
                k=centroids.shape[0],
            )
            wire_req2 = serialize_tiled_second_round_request(req2)
            total_sent_bytes += sum(len(b) for b in wire_req2.encrypted_query_50) + sum(
                len(b) for b in wire_req2.encrypted_selectors
            )

            r1_time = time.perf_counter() - r1_start
            round1_times.append(r1_time)

            r2_start = time.perf_counter()
            score_tile_gen = tiled_batch_matching(
                cluster_matrix,
                wire_req2,
            ...
            def counted_batch_scores(score_iter=score_tile_gen):
                nonlocal total_recv_bytes
                for tile_bytes in score_iter:
                    total_recv_bytes += len(tile_bytes)
                    yield tile_bytes

            match_res, _ = check_tiled_score_batch_debug(
                counted_batch_scores(),
                secret_context,
                layout=party_a_state.layout,
                logical_width=cluster_matrix.shape[1],
                early_stop=config.get("early_stop", True),
            )
            predictions.extend(match_res.catches.tolist())
            r2_time = time.perf_counter() - r2_start
            round2_times.append(r2_time)
    else:
        # 单查询 / Mock 路径
        reusable_secret_context = None
        reusable_public_context_bytes = None
        reuse_context = not use_mock and config.get("reuse_context", True)
        if reuse_context:
            reusable_secret_context = create_ckks_context()
            reusable_secret_context.generate_relin_keys()

        for _, query_name in enumerate(names_A):
            if use_mock:
                # 随机生成查询签名（mock）
                query_sig_cluster = np.random.randn(config["el_cluster"])
                _query_sig_match = np.random.randn(config["el_match"])
                # 标准化（使用 B 的 scaler 参数）
                _query_norm_scaled = (query_sig_cluster - scaler_mean) / (
                    scaler_scale + 1e-8
                )
                # 加密（模拟）
                # 实际应调用成员2的加密函数
                # 这里用字节大小模拟通信
                mock_ciphertext_cluster = b"mock_cipher_cluster" * (
                    config["el_cluster"] // 10
                )
                mock_ciphertext_match = b"mock_cipher_match" * (
                    config["el_match"] // 10
                )
                total_sent_bytes += len(mock_ciphertext_cluster) + len(
                    mock_ciphertext_match
                )
                ...
                def counted_scores(score_iter=raw_scores):
                    nonlocal total_recv_bytes
    ...
    comm_sent_mb = total_sent_bytes / 1024**2
    comm_recv_mb = total_recv_bytes / 1024**2
    comm_total_mb = (total_sent_bytes + total_recv_bytes) / 1024**2

    comm_sent_kb_per_q = (
        ((total_sent_bytes / num_queries) / 1024) if num_queries > 0 else 0.0
    )
    comm_recv_kb_per_q = (
        ((total_recv_bytes / num_queries) / 1024) if num_queries > 0 else 0.0
    )
    comm_total_kb_per_q = (
        (((total_sent_bytes + total_recv_bytes) / num_queries) / 1024)
        if num_queries > 0
        else 0.0
    )
    ...
    result = {
        ...
        "communication_per_query_kb": {
            "sent": comm_sent_kb_per_q,
            "recv": comm_recv_kb_per_q,
            "total": comm_total_kb_per_q,
        },
